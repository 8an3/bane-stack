		...
		<div className="bg-background">
			<div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-24 lg:px-8">
				<div className="max-w-xl">
					...
					<p className="mt-2 text-sm text-muted-foreground">Check the status of recent orders, manage returns, and discover similar products.</p>
				</div>
					...
					<div className="space-y-20">
						{orders.map((order) => (
							<div key={order.id}>
								...
								<div className="rounded-lg bg-background px-4 py-6 sm:flex sm:items-center sm:justify-between sm:space-x-6 sm:px-6 lg:space-x-8">
									<dl className="flex-auto space-y-6 divide-y divide-gray-200 text-sm text-muted-foreground sm:grid sm:grid-cols-3 sm:gap-x-6 sm:space-y-0 sm:divide-y-0 lg:w-1/2 lg:flex-none lg:gap-x-8">
										...
										<div className="flex justify-between pt-6 font-medium text-foreground sm:block sm:pt-0">
											<dt>Total amount</dt>
											<dd className="sm:mt-1">{order.total}</dd>
										</div>
									</dl>
									<Button
										className="mt-6 flex w-full items-center justify-center rounded-md border border-border bg-background px-4 py-2 text-sm font-medium text-foreground shadow-sm hover:bg-background focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 sm:mt-0 sm:w-auto"
										variant="outline"
									>
										View Order
								...
								<table className="mt-4 w-full text-muted-foreground sm:mt-6">
									...
									<tbody className="divide-y divide-gray-200 border-b border-border text-sm sm:border-t">
											...
											<tr key={item.id}>
												...
												<td className="hidden py-6 pr-8 sm:table-cell">
													<div className="flex items-center">
														...
														{order.status === "delivered" && order.deliveredTo && <div className="ml-2 text-sm text-muted-foreground">to {order.deliveredTo}</div>}
													</div>
													...
													{order.status === "out-for-delivery" && order.estimatedDelivery && <div className="mt-1 text-sm text-muted-foreground">Arrives {order.estimatedDelivery}</div>}
												</td>
										...
										<div key={item.id} className="mt-6 flex space-x-6">
											<div className="flex-auto">
												<div className="flex items-center">
													{getStatusIcon(order.status)}
													<span className={`ml-2 text-sm font-medium ${getStatusColor(order.status)}`}>{getStatusText(order.status)}</span>
													{order.status === "delivered" && order.deliveredTo && <span className="ml-2 text-sm text-muted-foreground">to {order.deliveredTo}</span>}
												</div>
												{order.status === "delivered" && order.deliveryDate && (
												...
												{order.status === "out-for-delivery" && order.estimatedDelivery && <p className="mt-1 text-sm text-muted-foreground">Arrives {order.estimatedDelivery}</p>}
											</div>
											<Button variant="link" className="text-primary hover:text-primary p-0 h-auto">
												View
											</Button>
