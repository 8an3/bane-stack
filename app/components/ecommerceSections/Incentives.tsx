...
const IncentiveItem = ({ title = "Default Title", description = "Default description goes here.", icon = <CheckCircle2 className="h-8 w-8 text-primary" />, badgeText = "Popular", ctaText = "Learn More", variant = "default" }: IncentiveItemProps) => {
	return (
		<Card className="group transition-all hover:-translate-y-1">
			...
			<CardContent>
				<a href="#" className="inline-flex items-center text-sm text-primary hover:text-primary/80 transition-colors">
					{ctaText}
	...
	return (
		<div className={`grid gap-6 ${columns === 3 ? "md:grid-cols-3" : "md:grid-cols-4"} lg:gap-8`}>
			...
			{items.map((item, index) => (
				<IncentiveItem key={index} {...item} variant={variant} />
			))}
		</div>
	);
}
