...
export function PricingComparison() {
	return (
		<div className="max-w-4xl mx-auto grid grid-cols-3 gap-4 md:grid-cols-4">
			<div className="font-medium text-foreground dark:text-slate-200"></div>
			{plans.map((plan) => (
				<div key={plan.name} className="text-center">
					<div className="text-lg font-semibold mb-2 text-foreground dark:text-slate-200">{plan.name}</div>
					<div className="text-2xl font-bold mb-4 text-foreground  ">{plan.price}</div>
				</div>
			))}
			{features.map((feat) => (
				<React.Fragment key={feat}>
					<div className="py-2 font-medium text-foreground  ">{feat}</div>
					{plans.map((plan) => (
