import { CheckCircle2, Clock, AlertCircle, User, FileText, Mail } from "lucide-react";
import { cn } from "~/components/ui/utils";
import { Badge } from "~/components/ui/badge";
import { FeedListSimpleFeedWithIcons } from "./FeedLists";

type FeedItem = {
	id: string;
	type: "completed" | "in-progress" | "error" | "user" | "document" | "email";
	title: string;
	description: string;
	date: string;
	datetime: string;
...
export function FeedListMultipleItemTypes({ items }: { items: FeedItem[] }) {
	return (
								...
								<div>
									<span
										className={cn("flex h-8 w-8 items-center justify-center rounded-full ring-8 ring-white", {
											...
											"bg-purple-500": item.type === "document",
											"bg-primary": item.type === "email",
										})}
									>
										...
										{item.type === "user" && <User className="h-5 w-5 text-white" />}
										{item.type === "document" && <FileText className="h-5 w-5 text-white" />}
										{item.type === "email" && <Mail className="h-5 w-5 text-white" />}
									</span>
								</div>
								<div className="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
									<div>
										<p className="text-sm text-gray-800">
											{item.title}{" "}
											{item.badge && (
												<Badge variant={item.type === "completed" ? "success" : item.type === "error" ? "destructive" : "default"} className="ml-2">
													{item.badge}
												</Badge>
											)}
										</p>
