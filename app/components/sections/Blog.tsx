...
export function Blog({ title = "From the blog", desc = "Learn how to grow your business with our expert advice." }) {
	return (
		<div className="bg-background py-24 sm:py-32">
			<div className="mx-auto max-w-7xl px-6 lg:px-8">
				<div className="mx-auto max-w-2xl lg:mx-0">
					<h2 className="text-4xl font-semibold tracking-tight text-pretty text-foreground sm:text-5xl">{title}</h2>
					<p className="mt-2 text-lg/8 text-muted-foreground">{desc}</p>
				</div>
					...
					{posts.map((post) => (
						<article key={post.id} className="flex max-w-xl flex-col items-start justify-between">
							<div className="flex items-center gap-x-4 text-xs">
								...
								<a href={post.category.href} className="relative z-10 rounded-full bg-background px-3 py-1.5 font-medium text-muted-foreground hover:bg-background/70">
									{post.category.title}
								</a>
							</div>
							...
							<div className="relative mt-8 flex items-center gap-x-4 justify-self-end">
								<img alt="" src={post.author.imageUrl} className="size-10 rounded-full bg-background" />
