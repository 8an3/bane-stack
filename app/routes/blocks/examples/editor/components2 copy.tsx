...
export default function MonacoEditor({ code, sections, name, setName, viewSelected }) {
	...
	const ButtonBar = () => {
		return (
			<div className="flex items-center space-x-2">
				...
				<Button variant="ghost" size="sm" onClick={resetSelection} className="text-muted-foreground hover:text-foreground">
	...
	const [showCommand, setShowCommand] = useState(true); // New state to control visibility
	const [selectedItem, setSelectedItem] = useState(null); // Track selected item

	const handleSelect = (item) => {
		setName(item.value);
		setSelectedCode(item.path);
		setShowCommand(false); // Hide command after selection
	};

	const resetSelection = () => {
		setShowCommand(true); // Show command palette again
	...
	return (
		<div className="flex flex-col h-screen">
			...
			{showCommand && (
				<div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
					<div className="relative">
						<Command className="rounded-lg border shadow-md w-[80vw] max-w-2xl">
			...
			{/* Editor Area */}
			<div className={`h-full flex flex-col bg-background ${showCommand ? "opacity-50" : ""}`}>
				{/* Header with back button (only when not showing command) */}
				{!showCommand && (
					<div className={`bg-background border-border border-b px-4 py-2 flex items-center justify-between`}>
						<div className="flex items-center space-x-4">
				...
				{/* Status Bar (only when not showing command) */}
				{!showCommand && (
