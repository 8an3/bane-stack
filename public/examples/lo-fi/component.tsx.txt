...
function Component({ href, ...props }: React.ComponentProps<typeof Link>) {
  return <NavLink to={href} className="group flex flex-col gap-2" {...props} />
}
...
function ComponentContent({ ...props }: React.ComponentProps<typeof Atom>) {
  return (
    <Atom
      shade="50"
      className="bg-background/30 ring-muted flex aspect-video items-center justify-center rounded-lg p-4 ring *:w-full *:max-w-[70%]"
      {...props}
    />
  )
}
