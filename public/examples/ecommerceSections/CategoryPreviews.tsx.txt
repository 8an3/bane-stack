            ...
            {callouts.map((callout) => (
              <div key={callout.name} className="group relative">
                <img
                  ...
                  className="w-full rounded-lg bg-background object-cover group-hover:opacity-75 max-sm:h-80 sm:aspect-2/1 lg:aspect-square"
                />
                ...
                <p className="text-base font-semibold text-foreground">{callout.description}</p>
              </div>
            ))}
          </div>
        </div>
